...
f_g = 0.97;  % Base flow coefficient
...
D_qian = [R_qian * cos(pi/4) + W, R_qian * sin(pi/4)];
...
text(D_qian(1), D_qian(2), ' D_qian', 'FontSize', 12, 'Color', 'r');
text(D_hou(1), D_hou(2), ' D_hou', 'FontSize', 12, 'Color', 'black');

xlabel('X Axis');
ylabel('Y Axis');
title('P1 (Green) and P2 (Red) Regions');
legend('P1 Boundary', 'P1 Points', 'P2 hou', 'P2 hou');
hold off;
...
P2_x = linspace(min(P2_points(:,1)), max(P2_points(:,1)), 400);
P2_y = linspace(min(P2_points(:,2)), max(P2_points(:,2)), 400);
...
Nx_R = 400; Ny_R = 400;
x_R = linspace(P/2 , P/2+W, Nx_R);  
y_R = linspace(0, P/2, Ny_R);        
[X_R, Y_R] = meshgrid(x_R, y_R);

% **Ensure valid values at P/2 in `F_P2_grid`**
[~, idx] = min(abs(P2_X(1,:) - P/2)); 
F_P2_P_boundary = F_P2_grid(:, idx);  % Directly extract values at X=P/2
...
% **Figure 1: Flow field before interpolation (Cartesian coordinates)**
figure(2);
...
% **Figure 2: Interpolated complete flow field**
figure(3);
...
% **Figure 3: Grid shape comparison before and after interpolation (Cartesian coordinates)**
figure(4);
hold on;
...
% **Figure 4: Right-side flow field**
figure(5);
...
% **Find the maximum value of `F_combined` and its location**
[max_F, max_idx] = max(F_combined(:)); % Find max value
[max_row, max_col] = ind2sub(size(F_combined), max_idx); % Convert to indices
max_X = X_combined(max_row, max_col);
max_Y = Y_combined(max_row, max_col);
...
xlabel('\it X\rm, mm', 'FontName', 'Times New Roman', 'FontSize', 14, 'Interpreter', 'tex');
ylabel('\it Y\rm, mm', 'FontName', 'Times New Roman', 'FontSize', 14, 'Interpreter', 'tex');
zlabel('\it f\rm_{sub,\iti}', 'FontName', 'Times New Roman', 'FontSize', 14, 'Interpreter', 'tex');
title({'1 / 8 center subchannel', '<\itW>=\rm0.389 m/s'}, 'FontName', 'Times New Roman', 'FontSize', 14);
...
text(max_X-P/9.5+0.2, max_Y-1.5, max_F+0.4,  sprintf('\\it f\\rm_{c,\\iti}: %.2f', max_F), ...
    'FontSize', 14, 'FontName', 'Times New Roman', 'Color', 'r', 'Interpreter', 'tex');
scatter3(P/2, 0, 0.97, 200, [0.5, 1, 1], 'filled'); 
text(P/2-W, -2.3, 0.97+0.4,sprintf('\\it f\\rm_{g,\\iti}: %.2f', 0.97), ...
...
% **定义圆柱参数**
x_center = W;  % 圆柱中心 x 坐标
y_center = 0;  % 圆柱中心 y 坐标
...
% **转换为直角坐标**
X_cyl = R .* cos(Theta) + x_center; % **将圆心放置在 (W, 0)**
Y_cyl = R .* sin(Theta) + y_center;
...
X_bottom = radius * cos(theta_bottom) + x_center; % **调整 X 坐标**
Y_bottom = radius * sin(theta_bottom) + y_center;
...
fprintf('Interpolated flow field area (2D calculation): %.4f\n', actual_area);
fprintf('Flow field area weighted average: %.4f\n', area_weighted_avg);
...
% 在图中间标注 R² 值
text(x_center-0.2, y_center+0.1, sprintf('R^{2} = %.5f\nMSE = %.5f\nRMSE = %.5f', r2_norm, mse_norm, rmse_norm), ...
